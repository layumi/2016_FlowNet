...
for t=1:opts.batchSize:numel(subset)
  ...
  if strcmp(mode, 'train')
    ...
    state = accumulate_gradients(state, net, opts, batchSize, mmap,t) ;
  end
...
% -------------------------------------------------------------------------
function state = accumulate_gradients(state, net, opts, batchSize, mmap,t)
% -------------------------------------------------------------------------
for p=1:numel(net.params)
  ...
  switch net.params(p).trainMethod
    ...
    case 'rmsprop'
      thisDecay = opts.weightDecay * net.params(p).weightDecay ;
      thisLR = state.learningRate * net.params(p).learningRate ;
      der = net.params(p).der + thisDecay * net.params(p).value;
      state.r{p} = (1 - opts.gamma) * (der).^2 + opts.gamma * state.r{p};
      %der_constraint = net.params(p).der;
      %der_constraint(der_constraint>opts.constraint) = opts.constraint;
      %der_constraint(der_constraint<-opts.constraint) = -opts.constraint;
      net.params(p).value = net.params(p).value - thisLR * (1 / batchSize) * der ./ (sqrt(state.r{p})+1e-6);
    
     case 'adam'
      thisDecay = opts.weightDecay * net.params(p).weightDecay ;
      beta1 = opts.beta1;
      beta2 = opts.beta2;
      t = fix(t/opts.batchSize)+1;
      %t is minibatch
      der = net.params(p).der + thisDecay * net.params(p).value;
      thisLR = state.learningRate * net.params(p).learningRate * sqrt(1-beta2^t)/(1-beta1^t);
      state.m{p} = (1 - beta1) .* der + beta1 * state.m{p};
      state.v{p} = (1 - beta2) .* (der.^2) + beta2 * state.v{p};
      net.params(p).value = net.params(p).value - thisLR * state.m{p} ./(sqrt(state.v{p}) + 1e-8);
